...
class LevelsModels {
  ...
  static find(id) {
    return new Promise(async (resolve, reject) => {
      const query = `SELECT * FROM levels WHERE dtm = 0 AND id = ? LIMIT 1;`;
      try {
  ...
  static insert (ins = {}) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (Object.keys(ins).length > 0) result = await MysqlHelpers.queryTrx(conn, `INSERT INTO levels SET ?;`, [ins]);
  ...
  static update (updtUsr = {}) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (Object.keys(updtUsr).length > 0) result = await MysqlHelpers.queryTrx(conn, `UPDATE levels SET ? WHERE id = ?;`, [updtUsr, updtUsr.id]);
  ...
  static delete (id) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        result = await MysqlHelpers.queryTrx(conn, `DELETE FROM levels WHERE id = ?;`, [id]);
